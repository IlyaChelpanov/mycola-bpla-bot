...
_SUMMARY_SYSTEM = (
    "Ты подробно подводишь итоги переписки группового чата на русском. "
    "Разбери основные темы РАЗВЁРНУТО: каждую тему оформи отдельным пунктом с "
    "подходящим тематическим эмодзи в начале, дай 1–3 предложения сути с "
    "деталями, аргументами и тем, кто что говорил. Структурируй по подтемам. "
    "Будь содержательным, но без воды и без перебора."
)
_USER_SUMMARY_SYSTEM = (
    "Ты подробно подводишь итог тому, о чём писал конкретный участник чата, "
    "на русском. Разбери его темы РАЗВЁРНУТО: каждую оформи пунктом с "
    "подходящим тематическим эмодзи, дай 1–3 предложения сути с деталями и "
    "аргументами/позицией человека. Структурируй по подтемам. Содержательно, "
    "но без воды и без перебора."
)
...
# leans on authoritative sources rather than forums and blogs.
_SEARCH_GUIDANCE = (
    " Когда ищешь информацию в интернете, в первую очередь опирайся на "
    "авторитетные источники: официальную документацию и руководства "
    "(в том числе руководства по эксплуатации и ремонту от производителя), "
    "официальные сайты организаций и госорганов, технические стандарты "
    "и Википедию. Форумы, блоги, drive2, отзывы и Q&A-агрегаторы используй "
    "только если авторитетного источника не нашлось — и тогда прямо отметь, "
    "что это неофициальные данные. Не используй российские источники и сайты "
    "в доменной зоне .ru. В ответе ссылайся на самый авторитетный из "
...
def _run_summary(cfg, conn, system: str, transcript: str) -> str:
    return llm.generate(
        system, transcript,
        provider=cfg.provider, model=effective_model(conn, cfg),
        api_key=cfg.active_api_key(), max_tokens=1200,  # detailed summaries
        base_url=cfg.active_base_url(),
    )
